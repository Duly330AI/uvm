...
- ✅ Unit archive fields (is_archived, rchived_at, rchived_by) plus rchive(user) helper shipped in ackend/app/landlord/models.py.
- ✅ Validator MinValueValidator(0) for rea_sqm is in place (ackend/app/landlord/models.py:131).
- ✅ Portal views/templates for units and nested meters exist (ackend/app/landlord/views.py:457, config/urls.py:157).
- ✅ API throttles mirror the property module (100/hour read, 50/hour write) via PortalReadThrottle/PortalWriteThrottle.
- ✅ Automated coverage: ackend/app/landlord/tests/test_unit_model.py, 	est_unit_api.py, 	est_unit_meter_api.py.

The remainder of this review is preserved for historical context.
